...
export function DeleteCommunityButton({
  ...
  async function handleDelete() {
    ...
    if (!response.ok) {
      setError(data.error ?? "Could not delete community.");
      return;
    }
    ...
    <>
      {trigger ? (
        isValidElement<{ onClick?: React.MouseEventHandler }>(trigger) ? (
          ...
          <span role="button" tabIndex={0} onClick={() => setOpen(true)}>{trigger}</span>
        )
      ) : (
        <button
          type="button"
      ...
      <FormShell
        ...
        description={`Permanently remove "${communityName}" and all of its posts.`}
        onClose={() => !loading && setOpen(false)}
        className="max-w-md"
      >
        <div className="flex flex-col gap-4">
          <p className="text-sm text-white/75">
            This action cannot be undone. All members, posts, and community data will be removed.
          </p>
